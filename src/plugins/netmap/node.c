...
{
//printf("%s:%d I bet this is a problem\n", __func__, __LINE__);
  ...
  if (PREDICT_FALSE (n_free_bufs < VLIB_FRAME_SIZE))
	...
	vlib_buffer_alloc (vm, &nm->rx_buffers[thread_index][n_free_bufs],
			   VLIB_FRAME_SIZE);
      //_vec_len (nm->rx_buffers[thread_index]) = n_free_bufs;	// TODO
      vec_set_len (nm->rx_buffers[thread_index], n_free_bufs);	// TODO copied from af_packet/node.c
    }
      ...
      while (r)
	{
	  ...
	  while (r && n_left_to_next)
	      ...
	      while (data_len && n_free_bufs)
		{
		  ...
		  //_vec_len (nm->rx_buffers[thread_index]) = last_empty_buffer;	// TODO
		  ...
		  if (PREDICT_TRUE (first_b0 != 0))
		    {
		      ...
		      int scratch = vlib_trace_buffer (vm, node, next0, first_b0,
					 /* follow_chain */ 0);
			if ( scratch == 0 ) {
				printf("something up in netmap town\n");
			}
